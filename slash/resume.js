    ...
    run: async (client, interaction) => {
        ...
        if (!interaction.member.permissions.has('ManageMessages') && !interaction.member.roles.cache.some((r) => r.name === "Giveaways")) {
            return interaction.reply({
                content: ':x: Vous avez pas les permissions administrateur pour mettre en reprendre un giveaways.',
                ephemeral: true
            });
        }
        ...
        client.giveawaysManager.unpause(giveaway.messageId)
            ...
            .then(() => {
                // Success message
                interaction.reply(`**[Le giveaway](https://discord.com/channels/${giveaway.guildId}/${giveaway.channelId}/${giveaway.messageId})** a repris!`);
            })
            .catch((e) => {
                interaction.reply({
                    content: e,
